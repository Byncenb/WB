import { BasicUnit, basicAttack, heal, strongAttack } from "./unit";
...
let autoBattleInterval: number | null = null;
...
function renderUnits(): void {
                    ...
                    `<div class="unit">
                        ...
                        <div>
                            ${unit.abilities
                                .map(
                                    (ability, i) =>
                                        `<button onclick="selectSkill(${index}, ${i})">${ability.name} (${ability.cost} MP)</button>`
                                )
                                .join("")}
                        </div>
                    </div>`
    ...
    if (enemyDiv) {
        enemyDiv.innerHTML = enemyUnits
            .map(
                (unit, index) =>
                    `<div class="unit" onclick="selectTarget(${index})">
...
function selectSkill(unitIndex: number, skillIndex: number): void {
    selectedUnitIndex = unitIndex;
    selectedSkillIndex = skillIndex;
}

// Выбор цели для способности
function selectTarget(targetIndex: number): void {
    if (selectedUnitIndex !== null && selectedSkillIndex !== null) {
        ...
        const target =
            playerUnits[selectedUnitIndex].abilities[selectedSkillIndex].name === "Лечение"
...
(window as any).selectTarget = selectTarget;
(window as any).clearLog = clearLog;
(window as any).toggleAutoBattle = toggleAutoBattle;
