// Интерфейс для описания расхода
interface Expense {
    id: string;
    category: string;
    amount: number;
...
// Интерфейс для отчета по категориям
interface CategoryReport {
    category: string;
    totalAmount: number;
}

// Интерфейс для общего отчета
...
class ExpenseManager {
    private expenses: Expense[] = []; // Список всех расходов
    private categories: Set<string> = new Set(); // Уникальные категории расходов
    ...
    addExpense(category: string, amount: number, date: string): void {
    ...
    private loadFromStorage(): void {
        ...
        if (data) {
            try {
                this.expenses = JSON.parse(data).map((expense: Expense) => ({
                    ...expense,
